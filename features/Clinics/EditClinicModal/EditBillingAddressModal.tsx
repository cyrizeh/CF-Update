...
const EditBillingAddressModal = ({ isOpen, clinic, updateDetails, setIsOpen }: EditClinicProps) => {
  ...
  const onSubmit = (data: any) => {
    updateDetails(editClinicAddressMapper(data, clinic.id))
      ...
      .catch(error => errorHandler(error, handleError, 'billingAddress'));
  };
    ...
    <div ref={rootRef}>
      <Modal root={rootRef.current ?? undefined} show={isOpen} size="md" onClose={onCloseModal}>
        <div className="flex items-center justify-between p-5">
          <div></div>
          <div className="text-3xl font-light">{t('edit_billing')}</div>
