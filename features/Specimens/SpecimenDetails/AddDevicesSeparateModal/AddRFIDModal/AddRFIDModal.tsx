...
import { AddDevicesModalFormValues, RFIDType } from '@/types/view/AddSpecimanModal.type';
...
const AddRFIDModal = ({ isOpen, onClose, index }: AddSpecimenModalWithStepsTypes) => {
  ...
  const { setValue, watch, trigger } = useFormContext<AddDevicesModalFormValues>();
  ...
  const handleSave = (data: EditRFIDFormValues) => {
    setValue(`devices.${index}.rfidType`, data.rfidType);
    setValue(`devices.${index}.rfId`, data.rfId);
    setValue(`devices.${index}.rfidReader`, data.rfidReader);
    setValue(`devices.${index}.rfidReaderId`, data.rfidReaderId);
    setValue(`devices.${index}.rfidItemType`, data.rfidItemType);
    trigger([
      `devices.${index}.rfidType`,
      `devices.${index}.rfId`,
      `devices.${index}.rfidReader`,
      `devices.${index}.rfidReaderId`,
      `devices.${index}.rfidItemType`,
    ]);
    onClose();
  };
  ...
  useEffect(() => {
    if (isOpen) {
      // Fetch existing RFID data for the specified index from devices
      const existingData = watch(`devices.${index}`);
