...
import { AddSpecimanModalFormValuesUpdateLodation } from '@/types/view/AddSpecimanModal.type';
...
const useUpdateCaneLocation = (isOpen: boolean) => {
  ...
  const { setValue } = useFormContext<AddSpecimanModalFormValuesUpdateLodation>();
  ...
  const clearAllListsWithOutfacilities = () => {
    ...
    setValue('caneData.vaultId', '');
    setValue('caneData.tankId', '');
    setValue('caneData.pieId', '');
    setValue('caneData.canisterId', '');
    setValue('caneData.slotId', '');
  };
