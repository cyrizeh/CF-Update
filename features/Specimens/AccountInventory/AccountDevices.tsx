import { useGetAccountDevices } from '@/api/queries/speciment.queries';
...
import { fdaEligibilityTypesFilter, ReactiveTypeListFilter, tissueTypeListFilter } from '@/constants/filters';
...
const Devices = ({ patientIdProps }: { patientIdProps?: string }) => {
  ...
  const { data: specimensData, isLoading } = useGetAccountDevices({
  ...
  const filtersConfig: Array<Filter & { show: boolean }> = [
    ...
    { type: 'date', value: 'freezeDate', placeholder: 'Freeze Date', label: 'Freeze Date', show: true },
    {
      type: 'date',
      value: 'receiptDate',
