...
function SignUpPage() {
  ...
  useEffect(() => {
    const fetchContactData = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_DEV_URL}/Signup/${router.query.ref}`);
  ...
  const onSubmit = (data: ViewTypes.SignupFormPropsFormValues) => {
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_BASE_DEV_URL}/Signup/${router.query.ref}`, {
