...
import { useGetComparePlansByAdmin, useGetStoragePlanByAdmin } from '@/api/queries/patient.queries';
...
const ReviewStoragePlan = () => {
  ...
  const { data: storagePlan, mutate: refetchStoragePlan, isLoading } = useGetStoragePlanByAdmin(patientId);
  const { data: plans, isLoading: isPlansLoading } = useGetComparePlansByAdmin(patientId);
  const { selectStoragePlanByAdmin } = usePatientMutation(storagePlan?.patientId);
  ...
  const handleChange = async (plan: string) => {
    ...
    try {
      const { data: nextPlan } = await axiosInstance.get(
        `admin/patients/${patientId}/calculateNextPlan?storageDuration=${plan}`
      );
      if (nextPlan) {
        setSpecimens(
  ...
  if (isLoading || onboardingDataLoading || isPlansLoading) {
    return (
  ...
  const actionButtons = [
    {
      label: 'Back',
      onClick: () => {
        router.push(`/admin/patients/${patientId}/onboarding/confirm-details`);
      },
      icon: arrowBackIcon,
      isIconPostfix: false,
      isFlat: true,
    ...
    },
    {
      ...
      onClick: async () => {
        await selectStoragePlanByAdmin({ storageDuration: selectedPlan, patientId })
          .then(() => {
    ...
    <div>
      <div className="grid-rows-[repeat(3,_minmax(0,_auto)] md:grid-rows-[repeat(2,_minmax(0,_auto)] relative grid gap-8 rounded-lg border-[1px]  px-4 text-base font-normal shadow-md transition md:px-8 md:py-10 dark:border-teal-400  dark:bg-cryo-grey dark:text-neutral-50 ">
        <div className="max-w-[450px]">
          <div className="text-4xl font-light text-white md:flex-row md:items-center ">{t('storagePlan.title')} </div>
        </div>

        <div className="text-4 mb-3 font-light">
          {renderDescription(
            onboardingData?.steps.find(step => step.type === 'StoragePlanSelectionStep')?.description || ''
          )}
        </div>
...
ReviewStoragePlan.getLayout = PatientProfilePageLayout;
