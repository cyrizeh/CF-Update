...
export default function AdminBillingStatementsPage() {
  ...
  const { id, statementId } = query;
  const [data, setData] = useState<any | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<any | null>(null);

  const { getAdminPatientStatement } = useDocumentMutation();
  ...
  if (error && error?.response?.status == NOT_FOUND_STATUS_CODE) {
    return <NotFound text={t('notFound:billingStatementNotFound')} />;
  }
