...
import { useGetExtraProtectionSubscriptionPatientPlan } from '@/api/queries/patient.queries';
...
const AddOn = () => {
  ...
  const { data: extraProtectiondata, isLoading } = useGetExtraProtectionSubscriptionPatientPlan({ isPatient: true });
  const { eppChoice } = usePatientPaymentMutation();
  ...
  useEffect(() => {
    if (extraProtectiondata) {
      ...
      setNextBillingDate(formatDateWithSlashSeparator(extraProtectiondata?.nextBillingDate as string));
    }
  }, [extraProtectiondata]);

  const handleEppChoice = async (option: string) => {
    await eppChoice.trigger({ eppChoice: option }).catch(() => {
      toast.error('');
    });
  ...
  return (
    <>
      <div className="grid-rows-[repeat(4,_minmax(0,_auto)] md:grid-rows-[repeat(2,_minmax(0,_auto)] relative grid gap-8 rounded-lg border-[1px] px-4 py-8 text-base font-normal shadow-md transition md:px-8 md:py-10 dark:border-teal-400 dark:bg-cryo-grey dark:text-neutral-50 ">
...
AddOn.getLayout = (page: any) => <OnboardingLayoutWithNoSSR>{page}</OnboardingLayoutWithNoSSR>;
